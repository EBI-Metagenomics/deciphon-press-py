...
class LineReader:
    def __init__(self, stream: trio.abc.ReceiveStream, maxlen=2048):
        self._stream = stream
        self._generator = self.generate_lines(maxlen)
    ...
    async def readline(self):
        ...
        while line is None:
            more_data = await self._stream.receive_some(1024)
            if not more_data:
                return ""  # this is the EOF indication expected by my caller
            line = self._generator.send(more_data)
        return line.decode()
